...
object MosaicRoutes extends LazyLogging with KamonTraceDirectives {
  ...
  def mosaicProject(projectId: UUID)(implicit database: Database, ec: ExecutionContext, bec: BlockingExecutionContext): Route =
    pathPrefix("export") {
  ...
  def getProjectScenesHistogram(projectId: UUID)(implicit database: Database, ec: ExecutionContext, bec: BlockingExecutionContext): Route = {
    ...
    def correctedHistograms(sceneId: UUID, projectId: UUID): OptionT[Future, Vector[Histogram[Int]]] =
      withComputationContext { implicit ec => implicit bec =>
        sceneIdsFuture.flatMap { implicit sceneIds =>
    ...
    entity(as[Array[UUID]]) { sceneIds =>
      val scenesHistograms =
        withComputationContext { implicit ec => implicit bec =>
          Future.sequence(sceneIds
            .map(correctedHistograms(_, projectId)) // Array[OptionT]
            .map(_.value).toList // Array[Future[Option[...]]]
          ) // Future[Array[Option[...]]]
        }

      val mergedBandHistograms =
        withComputationContext { implicit ec => implicit bec =>
          scenesHistograms.map { scenes =>
