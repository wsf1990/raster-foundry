...
object TileSources extends LazyLogging {
  ...
  /** Given the data sources for some AST, determine the "data window" for
    ...
    */
  def fullDataWindow(rs: Map[UUID, RFMLRaster])
                    (implicit database: Database, bec: BlockingExecutionContext): OptionT[Future, (Extent, Int)] =
  ...
  /** Given a reference to a source of data, computes the "data window" of
    ...
    * histogram.
    */
  def dataWindow(r: RFMLRaster)(implicit database: Database, bec: BlockingExecutionContext): OptionT[Future, (Extent, Int)] =
    withComputationContext { implicit ec => implicit bec =>
      r match {
        ...
        case _ => OptionT.none[Future, (Extent, Int)]
      }
    }
  ...
  def globalSource(
    ...
    zoom: Int,
    r: RFMLRaster
  )(implicit database: Database, bec: BlockingExecutionContext): Future[Option[TileWithNeighbors]] =
    withComputationContext { implicit ec => implicit bec =>
      r match {
